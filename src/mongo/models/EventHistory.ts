...
import {GroupsModelType, GroupsModelResponse} from "./Group";
import {utcDate} from "../../common/common";

export interface EventHistoryDb<Snapshot extends DbEventModel | EventModelType = DbEventModel> {
	date: Date,
...
export interface PopulatedEventHistoryDb {
	date: Date,
	fieldName: keyof DbEventModel,
	changeUserId: UserModelResponse,
	eventId: Schema.Types.ObjectId,
	snapshotDescription: string,
	eventSnapshot: EventModelWithPopulatedChains
}

export interface EventHistoryResponseItem {
	date: Date,
	fieldName: keyof DbEventModel,
	changeUserId: UserModelResponse,
	eventId: Schema.Types.ObjectId,
...
export interface EventHistoryPopulatedItem {
	date: Date,
	fieldName: keyof EventModelType,
	changeUserId: UserModelResponse,
	eventId: Schema.Types.ObjectId,
	snapshotDescription: string,
	eventSnapshot: EventModelWithPopulatedChains,
...
interface EventSnapshotResponse extends Omit<FullResponseEventModel, 'calendar' | 'linkedFrom' | 'parentId'> {
	calendar: FullResponseEventModel['calendar'] | null,
	parentId: FullResponseEventModel | null,
	linkedFrom: FullResponseEventModel | null
}
...
interface EventSnapshotCreatedFullType extends EventSnapshotCreateOptionalType, CreateSnapshotDefaultFields {
}
...
export type EventHistorySnapshot<FieldName extends EventHistoryEditableFieldNames = any> =
...
export interface EventHistoryCreateType<Key extends EventHistoryEditableFieldNames = any> {
...
export type EventHistoryEditableFieldsMap<FieldName extends Fields> = {
	[PROP in FieldName]: Omit<EventHistoryCreateType<PROP>, 'changeUserId' | "date">
}

//Массив объектов, передаваемый в метод HistoryHelper.addToHistory
export type EventHistoryArrayToCreate<FieldNames extends Fields> = Array<EventHistoryEditableFieldsMap<FieldNames>[FieldNames]>
...
const EventSnapshotSchema = new Schema()
	.add(SnapshotRequiredSchema)
	.add({
		group: {
			type: Schema.Types.ObjectId,
			ref: "Group",
			autopopulate: true,
			get(value: HydratedDocument<GroupsModelType> | null): GroupsModelResponse | null {
				if (!value || !value?._id) {
					return null
				}
				return {
