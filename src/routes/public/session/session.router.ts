...
const handlers = {
  ...
  async registration(
    req: express.Request<any, any, RegistrationRequestBody>,
    res: express.Response<CustomResponseBody<null>>
  ) {
    try {
      if (req.cookies['accessToken']) {
        return res.status(403).json({
          ...
          info: {
            message:
              'У текущего пользователя уже есть сессия, регистрация невозможна.',
            type: 'error',
          },
        });
      }

      const { phone, password, name, surname } =
      ...
      if (candidate) {
        return res.status(400).json({
          ...
          info: {
            message:
              'Пользователь с указанным номером телефона уже зарегестрирован',
            type: 'error',
          },
        });
      }
      ...
      if (!passCheck.status || !!passCheck.message) {
        return res.status(400).json({
          ...
          info: {
            message:
              passCheck.message ||
              'Указан некорректный пароль для регистрации нового пользователя',
            type: 'error',
          },
        });
      }
      ...
      return res.status(500).json({
        ...
        info: {
          message:
            e?.message ||
            'Во время обработки регистрации нового пользователя на сервере произошла ошибка',
          type: 'error',
        },
      });
    }
  ...
  async confirm(
    req: express.Request,
    res: express.Response<CustomResponseBody<Omit<UserModelType, 'password'>>>
  ) {
    ...
    const userInfo = (await JWT.decode(
      accessToken
