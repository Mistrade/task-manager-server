...
export class HistoryHelper {
  ...
  public static sortOutSnapshot(
    instance: EventHistoryQuerySnapshot
  ): Required<EventHistoryQuerySnapshot> {
    return {
      _id: instance._id,
  ...
  public async getHistoryCount(
    ...
    const count: number = await EventHistoryModel.count({
      eventId,
      ...(filters || {}),
    });
  ...
  public buildResultHistoryItem(
    item: EventHistoryQueryResult
  ): EventHistoryQueryResult {
    return {
      date: item.date,
  ...
  public async getHistoryListByEventId(
    ...
    const eventHistoryList: Array<EventHistoryQueryResult> | null =
      await EventHistoryModel.find(
        {
          eventId,
          $or: [
  ...
  public async addToHistory<FieldNames extends EventHistoryEditableFieldNames>(
    ...
    if (!Array.isArray(arr)) {
      throw new ResponseException(
        ResponseException.createObject(
          400,
          'error',
          'Объектов для добавления в историю событий не получено'
        )
      );
    }
    ...
    try {
      await EventHistoryModel.insertMany(data);
    } catch (e) {
      throw new ResponseException(
        ResponseException.createObject(
          500,
          'error',
          'Не удалось записать изменения в хранилище истории'
        )
      );
    }
    ...
    await EventHistoryModel.deleteMany({
      ...
      throw new ResponseException(
        ResponseException.createObject(
          500,
          'error',
          'Не удалось удалить историю события'
        )
      );
    });
  ...
  public getSnapshotRequiredFields(
  ...
  public buildHistoryItem<FieldName extends EventHistoryEditableFieldNames>(
    ...
    return {
      ...
      fieldName,
      snapshotDescription:
        options?.customDescription || HistoryDescription[fieldName],
      eventSnapshot: {
