...
export class GroupsValidator {
    ...
    if (!isValidTitle) {
      throw new ResponseException(
        ResponseException.createObject(
          400,
          'warning',
          'Заголовок группы событий должен быть от 5 до 20 символов'
        )
      );
    }
  ...
  protected validateGroupColor(color: string): boolean {
    ...
    const isValidColor =
      colorRegExpRGBA.test(resultColor) || colorRegExpDefault.test(resultColor);
    ...
    if (!isValidColor) {
      throw new ResponseException(
        ResponseException.createObject(
          400,
          'warning',
          'Цвет должен быть в формате HEX или RGB/RGBA'
        )
      );
    }
