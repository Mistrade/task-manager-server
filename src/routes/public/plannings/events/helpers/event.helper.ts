...
export class EventHelper extends EventBuildHelper {
    ...
    if (!result) {
      throw new ResponseException(
        ResponseException.createObject(
          404,
          'error',
          'События по указанным фильтрам не найдены'
        )
      );
    }
  ...
  public async findAndUpdateEvent(
    ...
    const prevEvent: EventModelType | null = await EventModel.findOneAndUpdate(
      filters,
      query,
      options || {}
    );
  ...
  public async create(
      ...
      try {
        ...
        throw new ResponseException(
          ResponseException.createObject(
            500,
            'error',
            'Не удалось найти одного или нескольких пользователей, приглашенных к событию'
          )
        );
      }
    }
      ...
      try {
        ...
        throw new ResponseException(
          ResponseException.createObject(
            500,
            'error',
            'Не удалось найти родительское событие'
          )
        );
      }
    }
      ...
      try {
        ...
        throw new ResponseException(
          ResponseException.createObject(
            500,
            'error',
            'Не удалось найти событие, от которого производится клонирование'
          )
        );
      }
    }
    ...
    toHistoryArray.push(
      historyApi.buildHistoryItem('createdAt', createdEvent, {
        ...
        parentEvent: parentEvent?._id
          ? historyApi.getSnapshotRequiredFields(parentEvent)
          : null,
        linkedFrom: linkedFromEvent?._id
          ? historyApi.getSnapshotRequiredFields(linkedFromEvent)
          : null,
      })
    );
    ...
    if (parentEvent) {
      ...
      toHistoryArray.push(
        ...
        historyApi.buildHistoryItem('insertChildOfEvents', parentEvent, {
          insertChildOfEvents: [
            historyApi.getSnapshotRequiredFields(createdEvent),
          ],
        })
      );
    }
    ...
    if (inviteUsers && inviteUsers?.length) {
      ...
      const result: Array<HydratedDocument<EventInviteQueryType>> | null =
        await EventInviteModel.insertMany(
          ...
          inviteUsers.map((u) => ({
            ...
            accessRights:
              members?.find((user) => objectIdIsEquals(user._id, u._id))
                ?.accessRights || 'viewer',
          }))
        );
      ...
      if (!result || !result.length) {
        ...
        throw new ResponseException(
          ResponseException.createObject(
            500,
            'error',
            'Событие было создано, но отправить приглашения пользователям не удалось'
          )
        );
      }
      ...
      const invitesUserId: Array<EventModelInvitesObject> = result.map(
        (item): EventModelInvitesObject => ({
          userId: item.invitedUser,
          inviteId: item._id,
        })
      );
  ...
  public async remove(filters?: EventModelFilters): Promise<void> {
    ...
    const eventsForRemove: Array<HydratedDocument<EventModelType>> | null =
    ...
    if (!eventsForRemove || eventsForRemove.length === 0) {
      throw new ResponseException(
        ResponseException.createObject(
          404,
          'error',
          'Не найдены события для удаления'
        )
      );
    }
    ...
    if (onlyCreatorEventsIdList.length === 0) {
      throw new ResponseException(
        ResponseException.createObject(
          403,
          'error',
          'Вы не можете удалить эти события'
        )
      );
    }
   ...
   */
  public async getShortEventsArray(
    filters: RequestEventFilters
  ): Promise<Array<ReturnEventTypeAfterBuild<'short'>>> {
    ...
    if (!query) {
      ...
      throw new ResponseException(
        ResponseException.createObject(
          400,
          'error',
          'На вход ожидался объект с фильтрами'
        )
      );
    }
    ...
    const eventList: Array<HydratedDocument<EventModelType>> =
      await this.getEventList(query);
    performance.mark('end');
    const dur = performance.measure('time', 'start', 'end').duration;

    console.log(`длительность поиска ${eventList.length} событий: `, dur);
