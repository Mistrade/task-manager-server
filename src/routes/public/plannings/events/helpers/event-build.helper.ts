...
export class EventBuildHelper extends EventCheckingHelper {
   ...
   * @since 25.02.2023
   */
  public async buildQueryObjectFromFilters(
    ...
    const pushToResult$And = (data: AnyObject) => {
      result['$and'] = result['$and'] ? [...result['$and'], data] : [data];
    };
    ...
    if (findOnlyInSelectedGroups) {
      ...
      const selectedGroups: Array<HydratedDocument<GroupsModelType>> | null =
      ...
      if (!selectedGroups) {
        throw new ResponseException(
          ResponseException.createObject(
            404,
            'error',
            'Не удалось найти выбранные группы событий'
          )
        );
      }
   ...
   * @since 25.02.2023
   */
  public buildShortEventResponseObject(
    event: EventModelType
  ): ReturnEventTypeAfterBuild<'short'> {
    return {
      ...
      isLiked: objectIdInArrayOfAnotherObjectId(
        this.user._id,
        event.likedUsers
      ), //Находится ли событие в списке избранных
   ...
   * @since 25.02.2023
   */
  public buildDefaultEventResponseObject(
    event: EventModelType
  ): ReturnEventTypeAfterBuild<'default'> {
    //Проверяю есть ли в инвайтах юзер
    const invite = event.invites.find((value) =>
      value?.userId ? objectIdIsEquals(value.userId, this.user._id) : false
    );
    ...
    return {
      ...
      group: GroupsHelper.buildGroupItemForResponse(event.group), //группа событий, за которой закреплено событие
      link: event.link, //Ссылка для подключения
      isLiked: objectIdInArrayOfAnotherObjectId(
        this.user._id,
        event.likedUsers
   ...
   * @since 25.02.2023
   */
  public buildEvent<BuildType extends EventBuildTypes>(
    event: EventModelType,
    buildType: BuildType
  ): ReturnEventTypeAfterBuild<BuildType> {
    switch (buildType) {
      case 'default':
        //Если buildType === default, то соберется полная версия события
        return this.buildDefaultEventResponseObject(
          event
        ) as ReturnEventTypeAfterBuild<BuildType>;
      case 'short':
        //Если buildType === short, то соберется короткая версия события
        return this.buildShortEventResponseObject(
          event
        ) as ReturnEventTypeAfterBuild<BuildType>;
      default:
    ...
    switch (minimalRoots) {
      ...
      case 'any':
        ...
        return {
          ...
          $or: [{ userId: this.user._id }, { 'invites.userId': this.user._id }],
        };
      default:
  ...
  public setInviteGroupAndBuild<BuildType extends EventBuildTypes = 'default'>(
    ...
    const invite = event.invites.find((value) =>
      objectIdIsEquals(value.userId, this.user._id)
    );
   ...
   * @since 25.02.2023
   */
  public async resolveEventsGroupAndBuild<
    BuildType extends EventBuildTypes = 'default'
  >(
    ...
    if (minimalRoots === 'viewer') {
      resultArray = resultArray.filter((event) =>
        this.isCreatorOrViewer(event)
      );
    }
    ...
    if (!group) {
      throw new ResponseException(
        ResponseException.createObject(
          404,
          'error',
          `Не удалось найти группу событий с типом ${replaceInvitesToGroupType} у текущего пользователя`
        )
      );
    }

    //В массиве событий, которые остались после фильтров, заменяю группу событий там, где пользователь - участник
    const res: Array<ReturnEventTypeAfterBuild<BuildType> | null> =
      resultArray.map((event): ReturnEventTypeAfterBuild<BuildType> | null =>
        this.setInviteGroupAndBuild<BuildType>(event, group, buildType)
      );

    //По итогу фильтрую массив, на предмет удаления любых false значений
    return res.filter<DefaultEventItemResponse>(
      (value): value is DefaultEventItemResponse => !!value
    );
  }
}
