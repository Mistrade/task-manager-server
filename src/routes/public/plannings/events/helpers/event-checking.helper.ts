 ...
 * @extends EventStorageHelper
 */
export class EventCheckingHelper extends EventStorageHelper {
    ...
    if (!eventId) {
      throw new ResponseException(
        ResponseException.createObject(
          400,
          'error',
          'Некорректное значение параметра eventId'
        )
      );
    }
  }
  ...
  public checkUserRootsAndBuild<T extends CheckUserRootsReturnedTypes>(
    ...
    if (!event) {
      throw new ResponseException(
        ResponseException.createObject(
          500,
          'error',
          'Не удалось проверить права доступа к событию'
        )
      );
    }
    ...
    if (
      !buildEvent.accessRights ||
      !minimalRootsMap[minimalRoots].includes(buildEvent.accessRights)
    ) {
      throw new ResponseException(
        ResponseException.createObject(
          403,
  ...
  public isAdmin(e: EventModelType): boolean {
    ...
    return !!e.invites.find(
      (item) =>
        objectIdIsEquals(item.userId, this.user._id) &&
