...
import {
	UpdateEventGroup,
	UpdateEventIsLiked, UpdateEventLinkItem,
	UpdateEventMapTypes,
	UpdateEventPriority,
	UpdateEventStatus, UpdateEventTime,
...
import {AnyObject, HistoryHelper} from "../../history/helper/historyHelper";
import {EventHistoryCreateType, EventHistoryEditableFieldNames} from "../../../../mongo/models/EventHistory";
import {DbTaskPriorities, DbTaskStatuses, TaskStatusesObject} from "../../../../common/constants";
...
interface UpdateFnReturned {
	historyItems: Array<EventHistoryCreateType<EventHistoryEditableFieldNames>> | null,
	result: AnyObject
}
...
export class EventUpdateHelper extends EventValidator {
	...
	private updatePriority(
		...
		if (!v) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Некорректное значение поля: "Приоритет события"')
			)
		}
	...
	private updateStatus(
		...
		if (!v) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Некорректное значение поля: "Статус события"')
			)
		}
	...
	private updateIsLiked(
		...
		if (typeof value.data !== 'boolean') {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Некорректное значение поля: "Добавить в избранное"')
			)
		}
		
		const alreadyLiked = value.data && !!event.likedUsers.find((like) => objectIdIsEquals(like, this.user._id))
		
		if (alreadyLiked) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Событие уже в списке "Избранное"')
			)
		}
		
		const historyApi = new HistoryHelper(this.user)
		const description = value.data ? 'Добавлено в избранное' : 'Удалено из избранного'
		...
		return {
			result,
			historyItems: [
				historyApi.buildHistoryItem('isLiked', event, {
					isLiked: value.data
				}, {
					isPrivate: true,
	...
	private async updateGroup(
		...
		if (!objectIdIsEquals(event.userId._id, this.user._id)) {
			throw new ResponseException(
				ResponseException.createObject(403, 'error', 'Только создатель события может менять значение поля: "Группа событий"')
			)
		}
		
		const groupFromDb: HydratedDocument<GroupsModelType> | null = await GroupModel.findOne({
			_id: value.data,
		...
		if (!groupFromDb || !groupFromDb._id) {
			throw new ResponseException(
				ResponseException.createObject(404, 'error', 'Не удалось найти группу событий, на которую необходимо обновить значение события')
			)
		}
		
		if (groupFromDb.type === 'Invite') {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Вы не можете добавлять события в группу "Приглашения", в этой группе появляются события, когда другие пользователи дают вам доступ к своим событиям')
			)
		}
		...
		return {
			...
			historyItems: [
				historyApi.buildHistoryItem('group', event, {group: groupFromDb._id}, {isPrivate: true})
			]
		}
	}
	...
	private updateLink(
		...
		if (!v) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Некорректное значение поля: "Ссылка для подключения"')
			)
		}
		
		const historyApi = new HistoryHelper(this.user)
		const description: string | undefined = !!value.data ? undefined : 'Ссылка для подключения была удалена'
		return {
	...
	private updateStartDate(
		...
		if (!v) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Невалидная дата начала события')
			)
		}
		...
		if (newStartTime.isSameOrAfter(prevTimeEnd, 'second')) {
			...
			return {
				...
				historyItems: [
					...
					historyApi.buildHistoryItem('timeEnd', event, {timeEnd: resultTimeEnd}, {customDescription: description})
				]
			}
		}
	...
	private updateEndDate(
		...
		if (!v) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Некорректная дата завершения события')
			)
		}
		...
		if (newEndTime.isSameOrBefore(prevStartTime, 'second')) {
			throw new ResponseException(
				ResponseException.createObject(400, 'error', 'Дата завершения должна быть после даты начала события. Измените сначала дату начала события.')
			)
		}
	...
	public async updateEventInfoAndPushToHistory(value: UpdateEventMapTypes): Promise<void> {
		...
		if (!event) {
			throw new ResponseException(
				ResponseException.createObject(403, 'error', 'Недостаточно прав доступа для редактирования этого события')
			)
		}
		...
		if (result && result.result && Object.keys(result.result).length > 0) {
			try {
				...
				throw new ResponseException(
					ResponseException.createObject(500, 'error', 'Не удалось сохранить изменения')
				)
			}
			...
			if (result.historyItems && result.historyItems?.length > 0) {
				...
				try {
					...
					throw new ResponseException(
						ResponseException.createObject(500, 'error', 'Не удалось зафиксировать историю события')
					)
				}
			}
		}
